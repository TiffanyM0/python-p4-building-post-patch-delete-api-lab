...
def bakery_by_id(id):
    if request.method == 'GET':
        ...
        response_body = {
            'deleted' : True,
            'message' : 'Bakery has been sucessfully deleted'
        }
        response = make_response(jsonify(response_body.to_dict()), 200)
        return response
...
def get_baked_goods():
    if request.method == 'GET':
        ...
        db.session.commit()
        response = make_response(jsonify(new_baked_goods.to_dict()), 201)
        return response
